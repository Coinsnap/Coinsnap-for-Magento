...
use Coinsnap\Payment\Api\Result\InvoicePaymentMethod;
use Coinsnap\Payment\Api\Util\PreciseNumber;
...
class Invoice extends AbstractClient{
    public function createInvoice(
        ...
        ?array $metaData = null,
        ?InvoiceCheckoutOptions $checkoutOptions = null): \Coinsnap\Payment\Api\Result\Invoice 
    {
        ...
        if ($response->getStatus() === 200) {
            return new \Coinsnap\Payment\Api\Result\Invoice(
    ...
    public function getInvoice(string $storeId,string $invoiceId): \Coinsnap\Payment\Api\Result\Invoice {
        ...
        if ($response->getStatus() === 200) {
            return new \Coinsnap\Payment\Api\Result\Invoice(json_decode($response->getBody(), true, 512, JSON_THROW_ON_ERROR));
        } else {
